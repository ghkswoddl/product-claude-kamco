...
## 기능 6 — 기업지원프로그램 업무플로우 PRD 반영 (진단/온라인신청/매칭신청 + 맞춤매칭·찜)

배경: `file/기업지원프로그램_IA_재설계_개선안.md`를 근거로 기업지원프로그램의 진단·온라인신청·매칭신청 플로우를 개선. 상세 설계는 `C:\Users\tagi7\.claude\plans\sleepy-pondering-boot.md` 최상단 섹션에 기록됨.

사용자 확정 범위:
- 자가진단(`program-diagnosis`): 로그인 시 자동매칭(재무정보 재입력 없음), 비로그인은 기존 수기입력 유지
- 신규 `program-matched`(맞춤 매칭 랜딩화면) + 신규 `mypage-bookmarks`(관심 프로그램/찜) 추가
- `program-investor-match`(매칭신청): 기존 공용 `apply` 폼 재사용 유지, 이미 신청한 경우 상태 인라인 표시만 추가
- `apply` Step1: 자동반영 안내 캡션 한 줄만 추가

### Work 분할 (5개 서브에이전트, 공유 인프라 선행 후 병렬)
1. M1(선행, 공유 JS/IA): IA/AUTH_IA 배열에 새 항목 추가, 찜 토글 함수, 매칭엔진 데이터+렌더함수, 매칭신청 상태 플래그+렌더함수, 홈 화면 버튼 1곳 goto 변경, apply Step1 캡션 추가
2. M2(M1 이후): `program-matched` 신규 화면 마크업
3. M3(M1 이후, M2와 병렬): `mypage-bookmarks` 신규 화면 마크업
4. M4(M1 이후, 병렬): `program-diagnosis` 로그인 자동 프리필 + `program-overview` 찜 버튼
5. M5(M1 이후, 병렬): `program-investor-match` 매칭상태 배너 마크업

### 검증 방법
Browser로 비로그인/기업회원 로그인 양쪽에서 전체 플로우 확인: 맞춤매칭 자동노출, 진단 자동프리필, 찜 토글↔관심프로그램 목록 동기화, 매칭신청 후 상태 배너 전환, GNB/모바일메뉴/사이트맵/마이메뉴에 신규 항목 노출. 375px/1280px 확인.
